...
//	MEDIA_POLL_RATE = 250,                 /**< in [Hz]             */
	MEDIA_POLL_RATE = 250,                 /**< in [Hz]             *///by aphero
...
//by aphero: 最终在h264.c中的rtp_send_data()调用pkth()，就是这个函数
			  ...
			  void *arg)
{
	...
	//info("list_append-[%d]\n",list_count(&vtx->sendq));//查看入队列,发送队列
	list_append(&vtx->sendq, &qent->le, qent);//添加到队列 * Append a list element to a linked list
	lock_rel(vtx->lock_tx);
			    ...
			    uint64_t timestamp)
{
	...
	/* Convert image */
	if (frame->fmt != (enum vidfmt)vtx->video->cfg.enc_fmt) {//转换格式
	...
	for (le = vtx->filtl.head; le; le = le->next) {//添加滤镜
	...
	err = vtx->vc->ench(vtx->enc, vtx->picup, frame, timestamp);//发送到编码器,编码器编码后，将包存放到packet_handler(),入队列
	if (err)
		goto out;
...
static int vtx_debug(struct re_printf *pf, const struct vtx *vtx)
{
	...
	//err |= re_hprintf(pf, "     source: %s %u x %u, fps=%.2f"//by aphero
	//		  " frames=%llu\n",
	//		  vtx->vsrc ? vidsrc_get(vtx->vsrc)->name : "none",//by aphero
...
static int vrx_debug(struct re_printf *pf, const struct vrx *vrx)
{
	...
	err |= re_hprintf(pf, " rx: decode: %s %s\n",
			  ...
			  vidfmt_name(vrx->fmt));
	err |= re_hprintf(pf, "     vidisp: %s %u x %u frames=%llu recv-frames=%d\n",
			  vrx->vidisp ? vidisp_get(vrx->vidisp)->name : "none",
			  vrx->size.w, vrx->size.h,
			  vrx->stats.disp_frames,vrx->frames);
