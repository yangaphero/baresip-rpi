			...
			int pix_fmt)
{
	...
	st->ctx->thread_count = 1;//by aphero
	
	/* params to avoid libavcodec/x264 default preset error */
	if (st->codec_id == AV_CODEC_ID_H264) {

		//av_opt_set(st->ctx->priv_data, "profile", "baseline", 0);//by aphero
...
st->ctx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER;//by aphero,添加后ffmpeg会将sps pps存入到extradata中，
	...
	err = av_frame_get_buffer(st->pict, 32);//by aphero
    if (err < 0)    debug("avcodec: Could not allocate the video frame data\n");
#endif
	   ...
	   uint64_t timestamp)
{
	...
	ret = av_frame_make_writable(st->pict);//by aphero
