	...
	int width, int height, int stride)
{
	...
	info("omx:1-omx port definition: h=%d w=%d s=%d sh=%d\n",
	...
	portdef.format.video.nFrameWidth = width;
	portdef.format.video.nFrameHeight = height;
	(void)stride;//by aphero
	portdef.format.video.nStride = width;//需32倍　　1920 设置为０,1080p不出错 原portdef.format.video.nStride = stride;
	portdef.format.video.nSliceHeight = 0;//需16倍　设置为０，原portdef.format.video.nSliceHeight = height;
	portdef.format.video.xFramerate = 25 << 16;
	portdef.bEnabled = 1;
	...
	info("omx:2-omx port definition: w=%d h=%d s=%d sh=%d　bEnabled＝%d\n",	
	portdef.format.video.nFrameWidth,
	portdef.format.video.nFrameHeight,	
	portdef.format.video.nStride,
	portdef.format.video.nSliceHeight,
	...
	void** pbuf, uint32_t* plen)
{
	...
	*plen = st->buffers[0]->nAllocLen;
	...
	if(first_packet){//by aphero 没什么作用
            st->buffers[0]->nFlags = OMX_BUFFERFLAG_STARTTIME;
            first_packet = 0;
    }else		st->buffers[0]->nFlags = OMX_BUFFERFLAG_TIME_UNKNOWN;
	return 0;
}
