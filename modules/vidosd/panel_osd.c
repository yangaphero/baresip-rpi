/**
 * @file panel.c  Video-osd filter -- panel
 *
...
static void rrd_append_osd(struct panel *panel, uint64_t val)
{
	if (!panel)
		return;
...
static int rrd_get_average_osd(struct panel *panel, uint64_t *average)
{
	if (!panel->rrdc)
		return ENOENT;
...
static void tmr_handler_osd(void *arg)
{
	struct panel *panel = arg;
	uint64_t now = tmr_jiffies();

	tmr_start(&panel->tmr, 2000, tmr_handler_osd, panel);
...
int panel_alloc_osd(struct panel **panelp, const char *label,
		unsigned yoffs, int width, int height)
{
	...
	tmr_start(&panel->tmr, 0, tmr_handler_osd, panel);
...
static void overlay_osd(struct vidframe *dst, unsigned yoffs, struct vidframe *src)
{
	uint8_t *pdst, *psrc;
	unsigned x, y;
...
static int draw_text_osd(struct panel *panel, struct vidframe *frame)
{
	char buf[256];
	...
	/* Draw text by aphero*/
	if (re_snprintf(buf, sizeof(buf), "%s",panel->label) < 0)
		return ENOMEM;
	...
	overlay_osd(frame, panel->yoffs, f2);
...
static void dim_frame_osd(struct vidframe *frame, unsigned yoffs, unsigned height)
{
	unsigned x, y;
	uint8_t *p;
	bool lower = (yoffs > 0);
...
static void draw_graph_osd(struct panel *panel, struct vidframe *frame)
{
	uint64_t avg;
	unsigned y0 = panel->yoffs;
	size_t i;

	if (rrd_get_average_osd(panel, &avg))
		return;
...
int panel_draw_osd(struct panel *panel, struct vidframe *frame)
{
	int err;
	...
	dim_frame_osd(frame, panel->yoffs, panel->size.h);

	err = draw_text_osd(panel, frame);
	if (err)
		return err;
	//draw_graph_osd(panel, frame);//by aphero

	return 0;
}


void panel_add_frame_osd(struct panel *panel, uint64_t pts)
{
	if (!panel)
		return;

	if (panel->pts_prev) {
		rrd_append_osd(panel, pts - panel->pts_prev);
	}
